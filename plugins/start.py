...
async def start(client, message):
    if not await checkdb.is_user_exist(message.from_user.id):
        ...
        name = await client.ask(message.chat.id, "<b>Welcome To Video Disk.\n\nIts Time To Create Account On Video Disk\n\nNow Send Me Your Business Name Which Show On Website\nEx :- <code>Cinerip Movies</code></b>")
        if name.text:
        ...
        rm = InlineKeyboardMarkup([[InlineKeyboardButton("✨ Update Channel", url="https://t.me/CineripMovies")]])
...
async def stream_start(client, message):
    ...
    encoded_url = f"{LINK_URL}?CineripMovies={link}"
...
async def link_start(client, message):
    if not message.text.startswith(LINK_URL):
        return
    link_part = message.text[len(LINK_URL + "?CineripMovies="):].strip()
    try:
